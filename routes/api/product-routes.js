...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json("Testing Error");
  }
});

// get one product
// http://localhost:3001/api/products/(provide id)
...
router.put("/:id", (req, res) => {
  ...
  Product.update(req.body, {
    ...
    .then((productTags) => {
      ...
      const productTagsToRemove = productTags.filter(({ tag_id }) => !req.body.tag_ids.includes(tag_id)).map(({ id }) => id);
